...
public:
	...
	void BindNativeInputAction(const UDataAsset_InputConfig* InInputConfig, const FGameplayTag& InInputTag,
	    ETriggerEvent InTriggerEvent, UserObject* InUserObject, CallbackFunc InCallback);
...
inline void UWarriorInputComponent::BindNativeInputAction(const UDataAsset_InputConfig* InInputConfig,
    const FGameplayTag& InInputTag, ETriggerEvent InTriggerEvent, UserObject* InUserObject, CallbackFunc InCallback)
{
	...
	if (FoundAction)
	{
		BindAction(FoundAction, InTriggerEvent, InUserObject, InCallback);
	}
}
template <class UserObject, typename CallbackFunc>
